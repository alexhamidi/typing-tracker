...
async def record(key_code: str, file: UploadFile = File(...)):
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        ...
        # Find left index finger (tip = landmark 8)
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            if label == "Left":
                ...
                # Draw circle on finger position
                ...
                # Save image
                image_path = f"calibration.jpg"
                cv2.imwrite(image_path, image)

                # Read existing calibration data
                calibration_data = {}
                try:
                    with open("keyboard_calibration.txt", "r") as f:
                        for line in f:
                ...
                # Update with new position (overwrites if exists)
                calibration_data[key_code] = (str(x), str(y))

                # Write back all data
                with open("keyboard_calibration.txt", "w") as f:
...
async def infer(key_code: str, file: UploadFile = File(...)):
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        ...
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            label = handedness.classification[0].label  # "Left" or "Right"
            prefix = "r" if label == "Left" else "l"  # Swap because camera is mirrored
    ...
    # Find closest finger to key position
    closest_finger = None
    min_distance = None
    ...
    # Draw key position on image
    cv2.circle(image, (key_x, key_y), 15, (0, 0, 255), 3)  # Red circle for key
